...
IMAG ='\IMAGCOH_OUTPUT';
addpath(genpath('I:\fieldtrip\FUNCTIONS\mass_uni_toolbox')); %for FDR correction
addpath('I:\fieldtrip\FUNCTIONS');
addpath([CWD,IMAG]);
...
for group_i = 1:2:(length(GROUPS)-1)
    fprintf('\n');
    A= load([CWD,'\ANALYSES\CONNECTIVITY_MATRICES\NI_Data\ALLDATA_' GROUPS{group_i}]);%active
    S= load([CWD,'\ANALYSES\CONNECTIVITY_MATRICES\NI_Data\ALLDATA_' GROUPS{group_i+1}]);%sham
...
MEANVALS = zeros(3,length(conditions),length(frequencies),length(times),64,2);
VARVALS = zeros(3,length(conditions),length(frequencies),length(times),64,2);
    ...
    for cond_i = 1:length(conditions)
        ...
        for time_i = 1:length(times)
            for freq_i = 1:length(frequencies)
                ...
                y = squeeze(median(squeeze(abs(S.ALLDATA.conditions.(conditions{cond_i}).(frequencies{freq_i})(:,time_i,:,:))),1));
                for chan_i = 1:64
                    MEANVALS(group_i,cond_i,freq_i,time_i,chan_i,1) = mean(x(chan_i,:));
                    MEANVALS(group_i,cond_i,freq_i,time_i,chan_i,2) = mean(y(chan_i,:));
                    VARVALS(group_i,cond_i,freq_i,time_i,chan_i,1) = var(x(chan_i,:));
                    VARVALS(group_i,cond_i,freq_i,time_i,chan_i,2) = var(y(chan_i,:));
                end
                avactconn = mean(MEANVALS(group_i,cond_i,freq_i,time_i,:,1),5);
                shactconn = mean(MEANVALS(group_i,cond_i,freq_i,time_i,:,2),5);
...
crit_p = zeros(size(p));
adj_p = zeros(size(p));
addpath(genpath('I:\fieldtrip\FUNCTIONS\mass_uni_toolbox'));
for group_i = [1,3,5];
    fprintf('.')
    for cond_i = 1:length(conditions)
        for freq_i = 1:length(frequencies)
...
addpath(genpath('I:\fieldtrip\PACKAGES\fieldtrip'))
addpath(genpath('I:\fieldtrip\PACKAGES\export_fig'))
%%
...
for group_i = [1,3,5]        
    count = 0;
    figure();set(gcf,'OuterPosition',[0 0 1920 1080],'Color',[1 1 1]);ha = tight_subplot(4,10,0.002);
    for cond_i = 1:length(conditions)

        for time_i = 2:3:length(times{1})
            count = count+1;
            temp = squeeze(TVALS2(group_i,cond_i,5,time_i,:,1));
            temp = [temp;0;0;0;0;0;0;0;0];
            dat.avg = temp;
            %search for sig electrodes
            siglabs = {};
            for i = 1:length(TVALS(group_i,cond_i,5,time_i,:,1));
                if abs(TVALS(group_i,cond_i,5,time_i,i,1)) >0;
            ...
            if count < 11
                ...
                title([num2str(times{1}(time_i)) ' ms'],'FontSize',fsize);axis off;
            else
                axes(ha(count));ft_topoplotER(cfg,dat);
            end
%         end
...
%             end
    ...
    %     imwrite(im,map,['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\NeuroImage_TMap_HeadPlots\BETA.gif'],'DelayTime',1,'LoopCount',inf)
    %saveas(gcf,['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\NeuroImage_TMap_HeadPlots\BETA_' times{time_i} '.eps'],'psc2');
    %print('-depsc','-tiff','-r300',['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\NeuroImage_TMap_HeadPlots\BETA_' GROUPS{group_i} '.eps'])
    %close;
    print2eps(['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\NeuroImage_TMap_HeadPlots\BETA_' GROUPS{group_i} '.eps'], gcf);
    %close;
end
%% Avg T-Map Connections
CONN_COUNT = zeros(3,length(conditions), length(frequencies), length(times), 2);
for group_i =   1:2:(length(GROUPS)-1)
    cd(['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\ABS\' GROUPS{group_i}]);
    for cond_i = 1:length(conditions)
        for freq_i = 1:length(frequencies)
            for time_i = 1:length(times)
                load(['I:\fieldtrip\ANALYSES\CONNECTIVITY_MATRICES\ABS\' GROUPS{group_i} ...
