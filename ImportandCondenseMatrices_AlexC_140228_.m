...
%             for dim1 = 1:size(delta_switchto_0to200,1) % poor form to have variable name like this but can't think of solution
                for dim2 = 1:size(delta_nondirleft_0to200,3)
                    for dim3 = 1:size(delta_nondirleft_0to200,4)
...
for starttime = START_T:100:END_T
    ...
    for cond_i = 1:4 %exclude mixrepeats as this condition is the reference condition
        fprintf('\n');
        ...
        for freq_i = 1:length(frequencies) 
            ...
            referencename  = strcat(frequencies(1,freq_i),'_',conditions(1,cond_i),'_',timeperiod);
            differencename = strcat(frequencies(1,freq_i),'_',conditions(1,cond_i),'_',timeperiod,'_diff');
            for dim1 = 1:size(delta_nondirleft_0to200,2)
                fprintf('.');
                for dim2 = 1:64
                    for dim3 = 1:64
                        eval([differencename{1,1} '(dim1,dim2,dim3) = ' filename{1,1} '(1,dim1,dim2,dim3) - ' referencename{1,1} '(2,dim1,dim2,dim3);']);
                    end
                end
            end
            toc
