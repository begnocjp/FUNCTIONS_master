...
%something happened with name_i, manual load below
load([wpms.dirs.CWD wpms.dirs.preproc '1101_mem_002' '_REPAIRED_AND_REFERENCED.mat']);
...
%     refdat = refdat_commonavg;
    ...
    for conditions_i = 1:length(conditions)
        ...
        for k = 1:length(indices)
            if indices(k,1) == 1;
                if size(refdat.trial{1,k},1) == 256
                    x = refdat.trial{1,k}(1:256,1:tlen);
                elseif size(refdat.trial{1,k},1) == 128
