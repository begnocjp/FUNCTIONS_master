%% CCM_ARTIFACT_PARTIAL_REJECTION_AUTO
...
% 
%  USEAGE:
%         ccm_artifact_partial_rejection_auto(wpms,name_i,lpfilter,lpfreq,min,max)
%  INPUTS:
...
%
%  Alexander Conley, Patrick Cooper & Aaron Wong, 2018
%  Functional Neuroimaging Laboratory, University of Newcastle
function ccm_artifact_partial_rejection_auto(wpms,name_i,lpfilter,lpfreq,min,max)
...
%     end
% end
cfg.artfctdef.threshold.channel   = 1:124;%don't include frontal-externals
cfg.artfctdef.threshold.min       = min;
cfg.artfctdef.threshold.max       = max;
cfg.trl                           = eogcorr.cfg.previous.previous{1,1}.previous.trl;
cfg.bpfilter                      = 'no';
cfg.artfctdef                     = 'partial';
cfg.artfctdef.minaccepttim        = 1.0;
