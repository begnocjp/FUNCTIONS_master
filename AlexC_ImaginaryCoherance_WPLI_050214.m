        ...
        for cond_i = 1:length(cond)
            for chx = 1:64
                for chy = chx+1:64
                    ...
                    %WPLI_ALL = zeros(size(complex_mw_tf_1,1),size(complex_mw_tf_1,2),'single');
...
%                         input    = ImagCoh;        % make everything imaginary
%                         outsum   = input;%nansum(input);      % compute the sum; this is 1 x size(2:end)
%                         outsumW  = abs(input);%(abs(input)); % normalization of the WPLI
%                         %if debias
...
%                     current_file = 'WPLI';
%                     mkdir([NWD,'\','IMAGCOH_OUPUT']);
%                     mkdir([NWD,'\','IMAGCOH_OUPUT','\',names{1,names_i}]);
%                     mkdir([NWD,'\','IMAGCOH_OUPUT','\',names{1,names_i},'\',cond{1,cond_i}])
%                     savefilename = [NWD,'\','IMAGCOH_OUPUT','\',names{1,names_i},'\',cond{1,cond_i}, ...
%                         '\',current_file,'_CH',num2str(chx), ...
%                         '_CH',num2str(chy),'.mat'];
%                     save(savefilename, 'WPLI_ALL', '-v7.3');
