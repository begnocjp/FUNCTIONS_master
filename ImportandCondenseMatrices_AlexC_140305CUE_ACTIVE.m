    ...
    for cond_i = 1:2 %exclude mixrepeats as this condition is the reference condition
        ...
        for freq_i = 1:length(frequencies) 
            ...
            for dim1 = 1:size(delta_nondirleft_1300to1500,2)
                ...
                for dim2 = 1:64
                    for dim3 = 1:64
                        str = [differencename{1,1} '(dim1,dim2,dim3) = ' filename{1,1} '(1,dim1,dim2,dim3) - mean_NONDIR(dim2,dim3);'];
                        %display(str)
                        eval(str);
                    end
                end
...
mkdir([CWD '\' 'ANALYSES\ACTIVE\DifferenceMatrices']);
cd([CWD '\' 'ANALYSES\ACTIVE\DifferenceMatrices']);
for starttime = START_ARRAY; %START_T:100END_T
    endtime = starttime + 200;
    timeperiod = [num2str(starttime) 'to' num2str(endtime)];
...
mkdir([CWD,'\ANALYSES\ACTIVE\SIM']);
cd([CWD,'\ANALYSES\ACTIVE\SIM']);
for starttime = START_ARRAY; %START_T:100END_T
    endtime = starttime + 200;
    timeperiod = strcat(num2str(starttime),'to',num2str(endtime));
    ...
    for cond_i = 1:2 % 1:4
        ...
        for freq_i = 1:length(frequencies)
            ...
            colorbar;
            savedir = strcat(CWD,'\ANALYSES\ACTIVE\SIM\');
%            mkdir(savedir);
