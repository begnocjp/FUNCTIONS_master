...
%   mean(load_subs{1,name_i}.timelock(1).avg(front_chan,N2_lat))      
    ...
    for plot_i = 1:length(condition_names)
        ...
        for roi_i = 1:length(chan)
            
            plot(DATA.timelock(plot_i).time,DATA.timelock(plot_i).avg(chan(roi_i).roi,:),colours{plot_i});
            ...
            saveas(gcf,[wpms.dirs.CWD wpms.dirs.TIMELOCK wpms.names{name_i} '\' wpms.names{name_i} '_' chan(roi_i).location '_TIMELOCK_ERP' condition]);
            saveas(gcf,[wpms.dirs.CWD wpms.dirs.TIMELOCK wpms.names{name_i} '\' wpms.names{name_i} '_' chan(roi_i).location '_TIMELOCK_ERP' condition],'bmp');
            saveas(gcf,[wpms.dirs.CWD wpms.dirs.TIMELOCK wpms.names{name_i} '\' wpms.names{name_i} '_' chan(roi_i).location '_TIMELOCK_ERP' condition,'.eps'],'psc2');
