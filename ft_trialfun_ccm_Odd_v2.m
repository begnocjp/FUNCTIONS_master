function [trl] = ft_trialfun_ccm_Odd_v2(cfg,name_i)
index = name_i;
...
% 'TRSP'  = end of trial
% 'trgt' = target tone
% 'resp' = motor response to the tone
% 'hab+' = initial tones
% 
...
for j = 1:(length(trigger)-1)
    ...
    if      trg1 == 'std+' & trg2 == 'resp' | ...
            trg1 == 'trgt' & trg2 == 'resp'
        
        RT_ARRAY = [RT_ARRAY; (sample(j+1)-sample(j))/cfg.hdr.Fs*1000];
        TRIAL_ARRAY = [TRIAL_ARRAY; trigger{j}];
    end
end

% ODD_ARRAY = {RT_ARRAY, TRIAL_ARRAY};

file = cfg.wpms.names{name_i};
f = fopen([file '_rt-data.csv'],'w');
fprintf(f,'Trial,RT\n');
for k = 1:length(TRIAL_ARRAY)
   speed = RT_ARRAY(k);
   fprintf(f, '%s,%d\n',TRIAL_ARRAY{k},RT_ARRAY(k));
end
fclose(f);
