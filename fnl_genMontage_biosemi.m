function [M] = fnl_genMontage_biosemi(CWD,PREPROC_OUTPUT,names,name_i)

% E = textread([CWD,'fnl_BioSemi72.loc'],'%s');
fid = fopen([CWD,'FUNCTIONS' filesep 'fnl_BioSemi72.loc']);
...
lab = cell((length(M.lab)-num_external_chans)-length(badchann),1);
theta = zeros((length(M.lab)-num_external_chans)-length(badchann),1);
phi = zeros((length(M.lab)-num_external_chans)-length(badchann),1);
