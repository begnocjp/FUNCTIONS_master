function fnl_saveOffIndividualConditions_MMN(wpms,conditions,cond,condition_code_values,name_i)
...
for refchan_i = 1:size(refchans)
    ...
    fprintf('Calculating: MMN_DevL  = %s - %s \n', cond{3}, cond{1});
    MMN_DevL = mean_data.(cond{3})-mean_data.(cond{1});
    tempdata = [];
    tempdata = refdat;
    tempdata.trial = {MMN_DevL};
    tempdata.trialinfo = mean_temptrinf;
    tempdata.sampleinfo = mean_tempsmpinf;
    y = length(tempdata.trial);
    tempdata.time = refdat.time(1:y);
    varname = ['MMN_DevL_data'];
    eval([varname,' = tempdata;']);
    ...
    fprintf('Calculating: MMN_DevS  = %s - %s \n', cond{2}, cond{1});
    MMN_DevS = mean_data.(cond{2})-mean_data.(cond{1});
    tempdata = [];
    tempdata = refdat;
    tempdata.trial = {MMN_DevS};
    tempdata.trialinfo = mean_temptrinf;
    tempdata.sampleinfo = mean_tempsmpinf;
    y = length(tempdata.trial);
    tempdata.time = refdat.time(1:y);
    varname = ['MMN_DevS_data'];
    eval([varname,' = tempdata;']);
    save([wpms.dirs.CWD wpms.dirs.preproc wpms.names{name_i} '_',varname,'.mat'],varname);
    cfg = [];
    cfg.ReferenceSites = {};
