...
% structure / whole sample mean , plot but change to plot for each
% condition
...
% correction"
% 5. Run two-sample t-tests to test difference between conditions and extract
%
...
frex=logspace(log10(2),log10(30),80); %change to match wavelet
...
%times = 0:4:1000; %determined by mw_tf window
%times = -200:4:1000;
channels = 1:128;
...
groups{1,1} = Control_group
groups{1,2} = Angelman_group_low_artrej_thresh
groupnames = {'HC','Angelman'};
...
% %Channels used as ROIs (must use multiple channels?) GSN - 128, will also
% %be used in clusters
...
%     count2 = 0;
%     count3 = 0;
%     cond_mwtf1 = zeros(80,251); %80 equals bins, 251 equals times variable, 
%     cond_mwtf2 = zeros(80,251);
%     cond_mwtf3 = zeros(80,251);
%     frontal_data_forttest = zeros(length(1:30),80,251);
%     parietal_data_forttest = zeros(length(1:30),80,251);
%     central_data_forttest = zeros(length(1:30),80,251);
...
%                 end
%             end
%             central_data_forttest(name_i,:,:) = tempdata3./(length(clusterelec));%*length(conditions));
%             
%         end      
%         %Average mw_tf everyone for conditions{block_i}
...
% %         subplot(1,2,1);contourf(times(1:251),frex(1:80),frontal_avg(1:80,1:251),50,'linecolor','none');caxis([-4 4]);colormap(parula);
% %         hold on;
...
% %         subplot(1,2,2);contourf(times(1:251),frex(1:80),parietal_avg(1:80,1:251),50,'linecolor','none');caxis([-4 4])
% %         
...
% load in each, like single frontal, repeat frontal, single paritetl etc...
% p_frontal = zeros(80,length(1:251)); %from 0 to 1000 creating p-value maps, full sample difference from zero
% p_parietal = zeros(80,length(1:251));
% p_central = zeros(80,length(1:251));
...
%Frontal
single_frontal = load([dataout 'data_' conditions{1} '_' clusternames{1} '_power.mat'], 'frontal_data_forttest');
repeat_frontal = load([dataout 'data_' conditions{2} '_' clusternames{1} '_power.mat'], 'frontal_data_forttest');
sf = single_frontal.frontal_data_forttest;
rf = repeat_frontal.frontal_data_forttest;
for freq = 1:80
    [~,p_frontal_sing(freq,:)] = ttest(squeeze(sf(:,freq,ttime_start:ttime_stop)))%'Alpha', 0.05); -is the data different from 0
end
for freq = 1:80
    [~,p_frontal_rept(freq,:)] = ttest(squeeze(rf(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
end

%Parietal
single_parietal = load([dataout 'data_' conditions{1} '_' clusternames{2} '_power.mat'], 'parietal_data_forttest');
repeat_parietal = load([dataout 'data_' conditions{2} '_' clusternames{2} '_power.mat'], 'parietal_data_forttest');
sp = single_parietal.parietal_data_forttest;
rp = repeat_parietal.parietal_data_forttest;
for freq = 1:80
    [~,p_parietal_sing(freq,:)] = ttest(squeeze(sp(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
end

for freq = 1:80
    [~,p_parietal_rept(freq,:)] = ttest(squeeze(rp(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
end

%Central
single_central = load([dataout 'data_' conditions{1} '_' clusternames{3} '_power.mat'], 'central_data_forttest');
repeat_central = load([dataout 'data_' conditions{2} '_' clusternames{3} '_power.mat'], 'central_data_forttest');
sc = single_central.central_data_forttest;
rc = repeat_central.central_data_forttest;
for freq = 1:80
    [~,p_central_sing(freq,:)] = ttest(squeeze(sc(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
end

for freq = 1:80
    [~,p_central_rept(freq,:)] = ttest(squeeze(rc(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
end
...
p_crit_frontal_sing = zeros(size(p_frontal_sing));
p_crit_frontal_rept = zeros(size(p_frontal_rept));
p_crit_parietal_sing = zeros(size(p_parietal_sing));
p_crit_parietal_rept = zeros(size(p_parietal_rept));
p_crit_central_sing = zeros(size(p_central_sing));
p_crit_central_rept = zeros(size(p_central_rept));

for freq = 1:80
    [~,p_crit_frontal_sing(freq,:)]=fdr_bky(p_frontal_sing(freq,:),thresh,'no');
end

for freq = 1:80
    [~,p_crit_frontal_rept(freq,:)]=fdr_bky(p_frontal_rept(freq,:),thresh,'no');
end

for freq = 1:80
    [~,p_crit_parietal_sing(freq,:)]=fdr_bky(p_parietal_sing(freq,:),thresh,'no');
end

for freq = 1:80
    [~,p_crit_parietal_rept(freq,:)]=fdr_bky(p_parietal_rept(freq,:),thresh,'no');
end

for freq = 1:80
    [~,p_crit_central_sing(freq,:)]=fdr_bky(p_central_sing(freq,:),thresh,'no');
end

for freq = 1:80
    [~,p_crit_central_rept(freq,:)]=fdr_bky(p_central_rept(freq,:),thresh,'no');
end

% logical test for signficant difference from zero for both groups together   
p_thresh_frontal_sing=zeros(size(p_frontal_sing));
p_thresh_frontal_sing(p_frontal_sing<p_crit_frontal_sing)=1;
save([dataout,'p_thresh_frontal_sing.mat'],'p_thresh_frontal_sing');

p_thresh_frontal_rept=zeros(size(p_frontal_rept));
p_thresh_frontal_rept(p_frontal_rept<p_crit_frontal_rept)=1;
save([dataout,'p_thresh_frontal_rept.mat'],'p_thresh_frontal_rept');

p_thresh_parietal_sing=zeros(size(p_parietal_sing));
p_thresh_parietal_sing(p_parietal_sing<p_crit_parietal_sing)=1;
save([dataout,'p_thresh_parietal_sing.mat'],'p_thresh_parietal_sing');

p_thresh_parietal_rept=zeros(size(p_parietal_rept));
p_thresh_parietal_rept(p_parietal_rept<p_crit_parietal_rept)=1;
save([dataout,'p_thresh_parietal_rept.mat'],'p_thresh_parietal_rept');

p_thresh_central_sing=zeros(size(p_central_sing));
p_thresh_central_sing(p_central_sing<p_crit_central_sing)=1;
save([dataout,'p_thresh_central_sing.mat'],'p_thresh_central_sing');

p_thresh_central_rept=zeros(size(p_central_rept));
p_thresh_central_rept(p_central_rept<p_crit_central_rept)=1;
save([dataout,'p_thresh_central_rept.mat'],'p_thresh_central_rept');
...
% Frontal Sing
        figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
        subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),front_mean_sing(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
        mask_data_front_sing=zeros(size(front_mean_sing(1:80,ttime_start:ttime_stop)));
        temp_data_front_sing=front_mean_sing(1:80,ttime_start:ttime_stop);
        mask_data_front_sing(p_thresh_frontal_sing(1:80,:)==1)=temp_data_front_sing(p_thresh_frontal_sing(1:80,:)==1);
        hold on;
        subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_frontal_sing(1:80,:),1,'linecolor','k','linewidth',3);caxis([-5 5])
        xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
        set(gca,'FontSize',18);title('Frontal');axis square;
        
% % Frontal Rept
%         figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
%         subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),front_mean_rept(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
%         mask_data_front_rept=zeros(size(front_mean_rept(1:80,ttime_start:ttime_stop)));
%         temp_data_front_rept=front_mean_rept(1:80,ttime_start:ttime_stop);
%         mask_data_front_rept(p_thresh_frontal_rept(1:80,:)==1)=temp_data_front_rept(p_thresh_frontal_rept(1:80,:)==1);
%         hold on;
%         subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_frontal_rept(1:80,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
%         xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
%         set(gca,'FontSize',18);title('Frontal');axis square;
%         
% % Parietal Sing
%         figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
%         subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),parietal_mean_sing(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
%         mask_data_parietal_sing=zeros(size(parietal_mean_sing(1:80,ttime_start:ttime_stop)));
%         temp_data_parietal_sing=parietal_mean_sing(1:80,ttime_start:ttime_stop);
%         mask_data_parietal_sing(p_thresh_parietal_sing(1:80,:)==1)=temp_data_parietal_sing(p_thresh_parietal_sing(1:80,:)==1);
%         hold on;
%        subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_parietal_sing(1:80,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
%         xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
...
% % Parietal Rept
%         figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
%         subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),parietal_mean_rept(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
%         mask_data_parietal_rept=zeros(size(parietal_mean_rept(1:80,ttime_start:ttime_stop)));
%         temp_data_parietal_rept=parietal_mean_rept(1:80,ttime_start:ttime_stop);
%         mask_data_parietal_rept(p_thresh_parietal_rept(1:80,:)==1)=temp_data_parietal_rept(p_thresh_parietal_rept(1:80,:)==1);
%         hold on;
%        subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_parietal_rept(1:80,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
%         xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
%         set(gca,'FontSize',18);title('Parietal');axis square;
% %         saveas(gcf,[dataout 'Incidental_Memory_TaskAvg_Power_005.pdf'],'pdf');
% %         close
...
% % Central Sing
%         figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
%         subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),central_mean_sing(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
%         mask_data_central_sing=zeros(size(central_mean_sing(1:80,ttime_start:ttime_stop)));
%         temp_data_central_sing=central_mean_sing(1:80,ttime_start:ttime_stop);
%         mask_data_central_sing(p_thresh_central_sing(1:80,:)==1)=temp_data_central_sing(p_thresh_central_sing(1:80,:)==1);
%         hold on;
%         subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_central_sing(1:80,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
%         xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
%         set(gca,'FontSize',18);title('Parietal');axis square;
% %         saveas(gcf,[dataout 'Incidental_Memory_TaskAvg_Power_005.pdf'],'pdf');
% %         close
...
% % Central Rept
%         figure();set(gcf,'Position',[0 0 1920 1080],'Color',[1 1 1]);
%         subplot(1,2,1);contourf(times(ttime_start:ttime_stop),frex(1:80),central_mean_rept(1:80,ttime_start:ttime_stop),50,'linecolor','none');caxis([-2 2]);colormap(parula)
%         mask_data_central_rept=zeros(size(central_mean_rept(1:80,ttime_start:ttime_stop)));
%         temp_data_central_rept=central_mean_rept(1:80,ttime_start:ttime_stop);
%         mask_data_central_rept(p_thresh_central_rept(1:80,:)==1)=temp_data_central_rept(p_thresh_central_rept(1:80,:)==1);
%         hold on;
%         subplot(1,2,1);contour(times(ttime_start:ttime_stop),frex(1:80),p_thresh_central_rept(1:80,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
%         xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
%         set(gca,'FontSize',18);title('Parietal');axis square;
% %         saveas(gcf,[dataout 'Incidental_Memory_TaskAvg_Power_005.pdf'],'pdf');
% %         close
...
tmp_data1 = zeros(78,80,251); % for 78 subjects
tmp_data2 = zeros(78,80,251);
tmp_data3 = zeros(78,80,251);
tmp_data4 = zeros(78,80,251);
tmp_data5 = zeros(78,80,251);
tmp_data6 = zeros(78,80,251);

for name_i = 1:length(all_names)
    for freq_i = 1:80
        for time_i = ttime_start:ttime_stop
            if p_thresh_frontal_sing(freq_i,time_i)==1
                tmp_data1(name_i,freq_i,time_i) = sf(name_i,freq_i,time_i);
            end
        end
    end
end
    ...
    for freq_i = 1:80
        for time_i = ttime_start:ttime_stop
            if p_thresh_frontal_rept(freq_i,time_i)==1
                tmp_data2(name_i,freq_i,time_i) = rf(name_i,freq_i,time_i);
            end
        end
    end
end
...
sf_avg = mean(tmp_data1,3);
rf_avg = mean(tmp_data2,3);
...
save([dataout 'frontal_single_0-1000_power.mat'],'sf_avg');
save([dataout 'frontal_repeat_0-1000_power.mat'],'rf_avg');
...
% % 
% % pvals_frontal = zeros(80,length(1:251));
% % tvals_frontal.tstat = zeros(80,length(1:251));
% 
% 
...
% 
% frontal_diff = zeros(80,length(1:251));
% 
% for freq_i = 1:80
%     for time_i = 1:251
...
%             [~,pvals_frontal(freq_i,time_i)] = ttest(single_frontal(:,freq,1:251),repeat_frontal(:,freq,1:251),'Alpha', 0.05);
%         end
%     end
% 
% end
...
% 
% pvals_parietal = zeros(80,length(1:251));
% tvals_parietal.tstat = zeros(80,length(1:251));
% 
...
% 
% parietal_diff = zeros(80,length(1:251));
% 
% for freq_i = 1:80
%     for time_i = 1:251
...
%             [~,pvals_parietal(freq_i,time_i)] = ttest(single_parietal(freq,1:251),repeat_parietal(freq,1:251),'Alpha', 0.05);
%         end
%     end
% 
% end
...
% %             p_young = zeros(80,length(1:9801));
% %             for freq = 1:80
% %                 [~,p_young(freq,:)] = ttest(squeeze(youngdata_forttest(:,freq,1:9801)));
% %             end
% %             
% %             p_old = zeros(80,length(1:9801));
% %             for freq = 1:80
