function patrick_ccm_plot_mean_ERP_ROIs(wpms,name_i,isreverse_ydir, conditions,condition,chan,ROI)
...
%         condition_name = condition_names{1,plot_i};
        plot(DATA.timelock(plot_i).time,mean(DATA.timelock(plot_i).avg(chan,:)),colours{plot_i});
    ...
    saveas(gcf,[wpms.dirs.CWD wpms.dirs.TIMELOCK wpms.names{name_i} '\' wpms.names{name_i} '_' ROI 'MEAN_TIMELOCK_ERP' condition,'.eps'],'psc2');
