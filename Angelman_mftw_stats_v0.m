...
for block_i = 1:length(conditions)
   clusters={{'19','10','20','11','4','12','5'},...
    ...
    cond_mwtf = ({cond_mwtf1,cond_mwtf1,cond_mwtf1})
    tempdata1 = zeros(bins,time_length);
    data_store = {tempdata1,tempdata1,tempdata1}
    out = {tempdata1,tempdata1,tempdata1}
    tempdata = {tempdata1,tempdata1,tempdata1}
    data_forttest1 = zeros(length(1:freq_range),bins,time_length);
    data_forttest = repmat({data_forttest1,data_forttest1,data_forttest1},[sample 1]) %here 78 is size of sample
...
var_sing = {,'p_frontal_sing','p_parietal_sing','p_central_sing'}
var_rept= {,'p_frontal_rept','p_parietal_rept','p_central_rept'}

ttest_sing = {'single_frontal','single_parietal','single_central'}
ttest_rept= {'repeat_frontal','repeat_parietal','repeat_central'}
...
% singf = single_frontal.frontal_data_forttest;
% reptf = repeat_frontal.frontal_data_forttest;
% for freq = 1:bins
%     [~,p_frontal_sing(freq,:)] = ttest(squeeze(singf(:,freq,ttime_start:ttime_stop)))%'Alpha', 0.05); -is the data different from 0
% end
% % 
% for freq = 1:bins
%     [~,p_frontal_rept(freq,:)] = ttest(squeeze(reptf(:,freq,ttime_start:ttime_stop)));%,'Alpha', 0.05); -is the data different from 0
% end
% 
% %Parietal
...
%p_crit_frontal_sing  = zeros(size(ttest_sing{1,1}));
% p_crit_frontal_rept  = zeros(size(ttest_rept{1,1}));
% p_crit_parietal_sing = zeros(size(ttest_sing{1,1}));
% p_crit_parietal_rept = zeros(size(ttest_rept{1,1}));
% p_crit_central_sing  = zeros(size(ttest_sing{1,1}));
% p_crit_central_rept  = zeros(size(ttest_rept{1,1}));

pcrit_sing = {p_crit,p_crit,p_crit}
pcrit_rept = {p_crit,p_crit,p_crit}

% ttest_sing{i}
% ttest_rept{i}


%p_vals = {p_frontal_sing,p_frontal_rept}
