    ...
    for freq_i = 1:length(frequencies)
        ...
        for thresh_i = 1:length(thresholds);
            ...
            for cond_i = 1:length(names)
                ...
                G     = weight_conversion(abs(ALLDATA.conditions.(names{cond_i}).(frequencies{freq_i})),'normalize');
    ...
    for freq_i = 1:length(frequencies)
        ...
        for thresh_i = 1:length(thresholds)
            for cond_i = 1:length(conditions)
                hold on;
                plot(thresholds(thresh_i),squeeze(mean(graphmeasures.clusteringcorr(:,freq_i,thresh_i,cond_i,time_i))),linecolours{freq_i,cond_i})
            end
        end
        %Path length
        subplot(2,2,2);
        ...
        for thresh_i = 1:length(thresholds)
            for cond_i = 1:length(conditions)
                hold on;
                plot(thresholds(thresh_i),squeeze(mean(graphmeasures.pathcorr(:,freq_i,thresh_i,cond_i,time_i))),linecolours{freq_i,cond_i})
            end
        end
        %Small World
        subplot(2,2,3);
        ...
        for thresh_i = 1:length(thresholds)
            for cond_i = 1:length(conditions)
                hold on;
                plot(thresholds(thresh_i),squeeze(mean(graphmeasures.smallworld(:,freq_i,thresh_i,cond_i,time_i))),linecolours{freq_i,cond_i})
            end
        end
        %Rich Club
        subplot(2,2,4);
        ...
        for thresh_i = 1:length(thresholds)
            for cond_i = 1:length(conditions)
                hold on;
                plot(thresholds(thresh_i),squeeze(mean(graphmeasures.rclubcorr(:,freq_i,thresh_i,cond_i,time_i))),linecolours{freq_i,cond_i})
            end
        end
    end
