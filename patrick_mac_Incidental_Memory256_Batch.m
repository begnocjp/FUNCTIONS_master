...
for name_i = length(wpms.names)   
    pre_trial = .5; %for target 1 sec on each side, for cue 
    post_trial = .5;
    trialfunction = 'patrick_Alerting_nocue'; %will need to change per task, each task has trial function, will need to edit function for ANT task
    file_ext = 'raw';
    trdat = patrick_Alerting_nocue(wpms,name_i,trialfunction,pre_trial,post_trial,file_ext);
    clear post_trial pre_trial trialfunction
    %patrick_ccm_ICA(wpms,name_i,trdat);        
end
