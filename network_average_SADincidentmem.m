...
dataout = 'E:\fieldtrip\WAVELET_OUTPUT_DIR\SADnetwork\';
...
%% Load in data
% For each condition, For each subject, load in data and create a 80(frex) x 6001(time) x 128(elec)structure
% Then split into 4 var for each freq bands and average over freq
% Then split each of the 4 freq var into 3 time window files and export as
% csv files


% for group_i = 1:length(cohorts)
    
    for cond_i = 1:length(conditions) % For each conditions
        ...
        for name_i = 1:length(names)
            ...
            for elec_i = 1:length(labels)
                fprintf('.');
                load([datain names{name_i} filesep conditions{cond_i} filesep names{name_i} '_' conditions{cond_i} '_' labels{elec_i} '_imagcoh_mwtf.mat'],'mw_tf');
                cond_mwtf1(elec_i,:,:) =  mw_tf;
                %             count1 = count1+1;
                %                 tempdata = tempdata+mw_tf;
            end
            
            % Split into freq bands
            deltatab = cond_mwtf1(:,1:18,:);
            thetatab = cond_mwtf1(:,19:35,:);
            alphatab = cond_mwtf1(:,36:49,:);
            betatab = cond_mwtf1(:,50:68,:);
            %         tab = {deltatab,thetatab,alphatab,betatab};
            
            % Average over Freq bands, and collapse array
            deltatab = squeeze(mean(deltatab,2));
            thetatab = squeeze(mean(thetatab,2));
            alphatab = squeeze(mean(alphatab,2));
            betatab = squeeze(mean(betatab,2));
            
            clear  cond_mw_tf1
            
            % Split into 3 time windows per freq band - NOT for 250 Hz - can just
            % use deltatab, thetatab, alphatab, betatab
            
            %         delta_early = deltatab(:,251:2501); delta_mid = deltatab(:,2501:3001); delta_late = deltatab(:,3001:3801);
            %         theta_early = thetatab(:,251:2501); theta_mid = thetatab(:,2501:3001); theta_late = thetatab(:,3001:3801);
            %         alpha_early = alphatab(:,251:2501); alpha_mid = alphatab(:,2501:3001); alpha_late = alphatab(:,3001:3801);
            %         beta_early = betatab(:,251:2501); beta_mid = betatab(:,2501:3001); beta_late = betatab(:,3001:3801);
            
            tab = {'deltatab','thetatab','alphatab','betatab'};
            tabs = {deltatab,thetatab,alphatab,betatab};
            
            for tab_i = 1:length(tab)
                filename = [names{name_i} '_' conditions{cond_i} '_' tab{tab_i} '.csv'];
                csvwrite(filename,tabs{tab_i});
            end
            
        end
        
        
        clear cond_mw_tf1 deltatab thetatab alphatab betatab tab
...
%        subplot(1,2,1);contour(times(801:6801),frex(1:68),p_thresh_young(1:68,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
        ...
        set(gca,'FontSize',18);title(['Frontal ' conditions{cond_i}]);axis square;
...
%         mask_data_old(p_thresh_old(1:68,:)==1)=temp_data_old(p_thresh_old(1:68,:)==1);
        subplot(1,2,2);contourf(times(1601:4001),frex(1:68),parietal_avg(1:68,1601:4001),50,'linecolor','none');caxis([-2 2])
...
%         subplot(1,2,2);contour(times(801:6801),frex(1:68),p_thresh_old(1:68,:),1,'linecolor','k','linewidth',3);caxis([-2 2])
        xlabel('Time (ms)','FontSize',18);ylabel('Frequency (Hz)','FontSize',18);
        set(gca,'FontSize',18);title(['Parietal ' conditions{cond_i}]);axis square;
        saveas(gcf,[dataout 'Incidental_Memory_' conditions{cond_i} '_Power.pdf'],'pdf');
        close
...
frontal = load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\results\Incidental_Mem30\Frontal_ttestdata.mat'], 'data_forttest');
parietal = load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\results\Incidental_Mem30\Parietal_ttestdata.mat'], 'data_forttest');

front = frontal.data_forttest;
back = parietal.data_forttest;

front_avg = load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\results\Incidental_Mem30\Frontal_power.mat'], 'avg');
parietal_avg = load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\results\Incidental_Mem30\Parietal_power.mat'], 'avg');
...
%         end
        ...
        save(['F:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\young_avg' blocknames{cond_i} '_' clusternames{cluster_i} '_power.mat'],'young_avg');
        save(['F:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\old_avg' blocknames{cond_i} '_' clusternames{cluster_i} '_power.mat'],'old_avg');
...
for cond_i =1:length(blocks)
    conditions=blocks{cond_i};
    load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\young_data' blocknames{cond_i} '.mat']);
    load(['E:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\old_data' blocknames{cond_i} '.mat']);
    
    for cond_i = 1:length(blocks{cond_i})
        fprintf('\n%s', conditions{cond_i})
...
%   saveas(gcf,['E:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\youngHeadplots' blocknames{block_i} 'v2.pdf'],'pdf');
  saveas(gcf,['E:\fieldtrip\WAVELET_OUTPUT_DIR\Movies\oldHeadplots' blocknames{cond_i} 'v2.pdf'],'pdf');
