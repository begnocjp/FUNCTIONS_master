...
for stim_i = 1:length(stim)
    ...
    for cond_i = [2 4]
        for time_i = 1:length(times)
            if stim_i == 1
                powerdata(stim_i,cond_i,time_i,:) = POWER_SHAM(cond_i,5,times(time_i),:);
            elseif stim_i == 2
                powerdata(stim_i,cond_i,time_i,:) = POWER_ACTIVE(cond_i,5,times(time_i),:);
            end
        end%
    end
end
...
saveas(gcf,'PowerPlotstDCs.eps','psc2');
