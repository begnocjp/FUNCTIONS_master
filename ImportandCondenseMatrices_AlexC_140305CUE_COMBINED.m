...
mkdir([CWD '\' 'ANALYSES\COMBINED\DifferenceMatrices']);
cd([CWD '\' 'ANALYSES\COMBINED\DifferenceMatrices']);
for starttime = START_ARRAY; %START_T:100END_T
    endtime = starttime + 200;
    timeperiod = [num2str(starttime) 'to' num2str(endtime)];
...
mkdir([CWD,'\ANALYSES\COMBINED\SIM']);
cd([CWD,'\ANALYSES\COMBINED\SIM']);
for starttime = START_ARRAY; %START_T:100END_T
    endtime = starttime + 200;
    timeperiod = strcat(num2str(starttime),'to',num2str(endtime));
    ...
    for cond_i = 1:2 % 1:4
        ...
        for freq_i = 1:length(frequencies)
            ...
            colorbar;
            savedir = strcat(CWD,'\ANALYSES\COMBINED\SIM\');
%            mkdir(savedir);
